...
### How do I update DClaw Policy?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp policy --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Policy without Kubernetes?

Yes, for local development:

```bash
cd dclaw-policy/frontend && npm run dev
cd dclaw-policy/backend && uvicorn dclaw_policy.main:app --reload
```
